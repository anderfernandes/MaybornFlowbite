    ...
    <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
      <ul class="flex flex-col p-4 md:p-0 mt-4 font-medium border border-default rounded-base bg-neutral-secondary-soft md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-neutral-primary">
        <cfif (len(m.content('filename')) eq 0)>
          <li><a href="/" class="block py-2 px-3 text-white bg-brand rounded-sm md:bg-transparent md:text-fg-brand md:p-0">Home</a></li>
          <cfelse>
          <li><a href="/" class="block py-2 px-3 text-heading rounded hover:bg-neutral-tertiary md:hover:bg-transparent md:border-0 md:hover:text-fg-brand md:p-0 md:dark:hover:bg-transparent">Home</a></li>
        </cfif>
        ...
        <cfif i.hasNext()>
          <cfloop condition="i.hasNext()">
            ...
            <cfif (FindNoCase(m.content('filename'), item.getURLTitle()) eq 0)>
              <li>
                <a href="#item.getUrl()#" class="block py-2 px-3 text-heading rounded hover:bg-neutral-tertiary md:hover:bg-transparent md:border-0 md:hover:text-fg-brand md:p-0 md:dark:hover:bg-transparent">#item.getMenuTitle()#</a>
              </li>
            <cfelse>
            <li>
              <a href="#item.getUrl()#" class="block py-2 px-3 text-white bg-brand rounded-sm md:bg-transparent md:text-fg-brand md:p-0">#item.getMenuTitle()#</a>
              </li>
            </cfif>
          </cfloop>
        </cfif>
